...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    usernames[user_index].username = malloc(10 * sizeof(char));
    read(client_fd, usernames[user_index].username, 10 * sizeof(char));
...
int read_from(int client_index, struct sockname *usernames) {
  ...
  char client_input[BUF_SIZE + 1 - username_size];
  int num_read = read(fd, &client_input, BUF_SIZE);
  strcat(buf, client_input);
